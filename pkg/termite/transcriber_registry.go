...
func (r *TranscriberRegistry) Release(modelName string) {
	...
	for _, closeFn := range orphans {
		if err := closeFn(); err != nil {
			r.logger.Warn("Error closing orphaned transcriber model",
				zap.String("model", modelName),
				zap.Error(err))
		}
	}
...
func (r *TranscriberRegistry) loadModel(info *TranscriberModelInfo) (transcribing.Transcriber, error) {
	...
	if item := r.cache.Get(info.Name); item != nil {
		r.logger.Debug("Transcriber model loaded by another goroutine",
...
func (r *TranscriberRegistry) Close() error {
	...
	for name, orphans := range r.evictedHandles {
		for _, closeFn := range orphans {
			if err := closeFn(); err != nil {
				r.logger.Warn("Error closing orphaned transcriber model during shutdown",
					zap.String("model", name),
					zap.Error(err))
			}
		}
